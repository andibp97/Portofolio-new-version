...
$(document).ready(function () {
  ...
  window.addEventListener('load', () => {
    const arrowOne = document.querySelector('.scroll-arrow-one');
    const arrowTwo = document.querySelector('.scroll-arrow-two');

    arrowOne.classList.add('arrow-animation');
    setTimeout(() => {
      arrowTwo.classList.add('arrow-animation');
    }, 200);

    const btn = document.querySelector('.scroll-more');
    btn.addEventListener('click', () => {
      const scrollStatus = document.querySelector('.scroll-status');
      scrollStatus.style.top = '85%';
      // Return back to top after page will have scrolled
      setTimeout(() => {
        scrollStatus.style.top = '15%';
      }, 1000);

      // Scroll to next page
      const sectionTwo = document.querySelector('#sectionTwo');
      let sectionTwoOffset = sectionTwo.offsetTop;
      let el = sectionTwo.offsetParent;

      while (el) {
        sectionTwoOffset += el.offsetTop;
        el = el.offsetParent;
      }
      setTimeout(() => {
        window.scroll({
          top: sectionTwoOffset,
          left: 0,
          behavior: 'smooth',
        });
      }, 500);
